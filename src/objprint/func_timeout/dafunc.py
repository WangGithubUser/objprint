...
def func_timeout(timeout: Union[float, FunctionType], func, args=(), kwargs=None):
    """
        func_timeout - Runs the given function for up to #timeout# seconds.

        Raises any exceptions #func# would raise, returns what #func# would return (unless timeout is exceeded),
        in which case it raises FunctionTimedOut
        ...
        If the timeout is exceeded, FunctionTimedOut will be raised within the context of the called function every two
        seconds until it terminates, but will not block the calling thread
        (a new thread will be created to perform the join). If possible, you should try/except FunctionTimedOut
    ...
    if thread.is_alive():
        ...
        class FunctionTimedOutTempType(FunctionTimedOut):
            def __init__(self):
                super().__init__(self, '', timeout, func, args, kwargs)

        function_timed_out_temp = type(
            'FunctionTimedOut' +
            str(
                hash(
                    "%d_%d_%d_%d" %
        ...
        stop_exception = function_timed_out_temp
        thread._stopThread(stop_exception)
...
def func_set_timeout(timeout: Union[float, FunctionType], allow_override=False):
    """
        func_set_timeout - Decorator to run a function with a given/calculated timeout (max execution time).
            Optionally (if #allowOverride is True), adds a parameter, "forceTimeout", to the
        ...
        @param allow_override <bool> Default False, if True adds a keyword argument to the decorated function,
            "forceTimeout" which, if provided, will override the #timeout. If #timeout was provided as a
            lambda / function, it will not be called.

        @throws FunctionTimedOut If time allowed passes without function returning naturally

        @see func_timeout
    """
    # Try to be as efficient as possible... don't compare the args more than once

    #  Helps closure issue on some versions of python
    default_timeout = copy.copy(timeout)

    is_timeout_a_function = issubclass(
            timeout.__class__, (types.FunctionType, types.MethodType, types.LambdaType,
    ...
    if not is_timeout_a_function:
        if not issubclass(timeout.__class__, (float, int)):
            try:
                ...
                raise ValueError(
                    'timeout argument must be a float/int for number of seconds, or a function/lambda which '
                    'gets passed the function arguments and returns a calculated timeout (as float or int). '
                    'Passed type: < %s > is not of any of these, '
                    'and cannot be converted to a float.' % timeout.__class__.__name__
                )

    if not allow_override and not is_timeout_a_function:
        # Only default_timeout provided. Simple function wrapper
...
#                return func_timeout(default_timeout, func, args=args, kwargs=kwargs)
        ...
        def _function_decorator(func):
            def _function_wrapper(*args, **kwargs):
                if 'forceTimeout' in kwargs:
                    use_timeout = kwargs.pop('forceTimeout')
                else:
